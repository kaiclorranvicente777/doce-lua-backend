...
from telegram.ext import (
    ApplicationBuilder,
    Application,        # necessário para avisar_inicio / enviar_despedida
    CommandHandler,
    MessageHandler,
...
def listar_enderecos_do_dia(arquivo, aba="Acompanhamento Doce Lua"):
    ...
    for row in range(2, ws.UsedRange.Rows.Count + 1):
        ...
        horario = str(ws.Range(f"F{row}").Value).strip() if ws.Range(f"F{row}").Value else ""
        ...
        if codigo and codigo != "#0000" and status and "concluído" not in status.lower():
            if data_entrega == hoje:
                enderecos.append((codigo, cliente, endereco, horario))
    ...
    try:
        ...
        for row in range(2, ws.UsedRange.Rows.Count + 1):
            ...
            if valor_ax == "#0000":
                ...
                # ✅ Ajuste para coluna Q (Valor Adicional) → só valores que não sejam frete
...
def listar_pedidos_abertos(arquivo, aba="Acompanhamento Doce Lua"):
    ...
    for row in range(2, ws.UsedRange.Rows.Count + 1):
        ...
        status = str(ws.Range(f"D{row}").Value).strip() if ws.Range(f"D{row}").Value else ""
        cliente = str(ws.Range(f"G{row}").Value).strip() if ws.Range(f"G{row}").Value else ""
        data_entrega = converter_data_excel(ws.Range(f"E{row}").Value)  # pega a data da coluna E
...
# corte 4
    ...
    if comando in ["endereços do dia", "endereços"]:
        ...
        if enderecos:
            for codigo, cliente, endereco, horario in enderecos:
                keyboard = [
        ...
        for entrada in entradas:
            if ":" not in entrada:
                ...
                if sugestao:
                    ...
                    await enviar_mensagem_segura(
                        context.bot, update.effective_chat.id,
                        "⚠️ Formato inválido. Use NomeColuna: Valor."
                    )
                continue
