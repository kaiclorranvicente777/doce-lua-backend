        ...
        for row in range(2, ws.UsedRange.Rows.Count + 1):
            ...
            if valor_ax == codigo:
                ...
                if coluna_excel in ["Q", "Y"]:
                    valor_num = str(valor).replace("R$", "").replace(",", ".").strip()
                    try:
                        ws.Range(f"{coluna_excel}{row}").Value = float(valor_num)
                    except:
...
def gerar_imagem_excel():
    ...
    try:
        ...
        wb = excel.Workbooks.Open(ARQUIVO)
        time.sleep(10)  # garante que carregue

        # Fecha qualquer outra planilha aberta automaticamente
        for wb_extra in list(excel.Workbooks):
        ...
        logging.info("Imagem do calendário gerada com sucesso.")
    except Exception as e:
...
def enviar_imagem_calendario():
    try:
        ...
        with open(IMAGEM_PATH, "rb") as img:
            files = {"photo": img}
            data = {"chat_id": CHAT_ID, "caption": "Calendário atualizado 🤖🗓️"}
...
async def fluxo_inicial(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if enviar_imagem_calendario():
        ...
        await enviar_mensagem_segura(context.bot, update.effective_chat.id, "⚠️ Erro ao enviar calendário.")
    ...
    # 3. Verificar se houve atualização
    ...
    for codigo, cliente in pendentes:
        await enviar_mensagem_segura(context.bot, update.effective_chat.id, f"⚠️ Pagamento pendente: {codigo} - {cliente}")
    ...
    if comando == "pagamentos pendentes":
        ...
        if pendentes:
            ...
            for codigo, cliente, data_entrega in pendentes:
                ...
                if data_entrega and isinstance(data_entrega, datetime.datetime):
            ...
            await enviar_mensagem_segura(context.bot, update.effective_chat.id, "⚠️ Nenhum pagamento pendente encontrado.")
        await enviar_menu_comandos(update, context)
        context.user_data["comando_confirmado"] = None
        return
            ...
            for nome_coluna, valor in pendentes:
                ...
                if coluna_excel:
                    ...
                    if ok:
                        ...
                        await enviar_mensagem_segura(
                            context.bot, update.effective_chat.id,
                            f"⚠️ Não foi possível atualizar o pedido {codigo}."
                        )
                else:
                    await enviar_mensagem_segura(
                        context.bot, update.effective_chat.id,
                        f"⚠️ A coluna '{nome_coluna}' não está disponível para atualização."
                    )
...
async def avisar_inicio(app):
    ...
    await enviar_mensagem_segura(
     ...
     # ✅ Gera e envia calendário logo após iniciar
    try:
        gerar_imagem_excel()
        if enviar_imagem_calendario():
            await enviar_mensagem_segura(app.bot, CHAT_ID, "📅 Calendário enviado com sucesso!")
        else:
            await enviar_mensagem_segura(app.bot, CHAT_ID, "⚠️ Erro ao enviar calendário.")
    except Exception as e:
        logging.error(f"Erro ao gerar/enviar calendário na inicialização: {e}", exc_info=True)
        await enviar_mensagem_segura(app.bot, CHAT_ID, "⚠️ Ocorreu um erro ao gerar/enviar o calendário.")
    ...
    if pendentes:
        await enviar_mensagem_segura(app.bot, CHAT_ID, "⚠️ Não esqueça de cobrar os pagamentos pendentes!")
        ...
        for codigo, cliente, data_entrega in pendentes:
            ...
            if data_entrega and hoje <= data_entrega:
                msg = (
                    f"Olá, {cliente} tudo bem? Sou o bot da Confeitaria Doce Lua 😊🤖\n"
                    f"Seu pedido {codigo} está confirmado e marcado como *Aguardando - Pagamento*.\n\n"
                    "Para que possamos dar continuidade e garantir a compra dos insumos para produção do seu pedido, "
                    "pedimos que nos envie o comprovante de pagamento de pelo menos 50% do seu pedido.\n\n"
                    "Caso já tenha realizado, por favor desconsidere esta mensagem.\n"
                    "Esta é uma mensagem automática, mas de muita importância.\n"
                    "Estamos à disposição para qualquer dúvida 💜"
                )
            else:
                msg = (
                    ...
                    "Caso já tenha realizado, por favor desconsidere esta mensagem.\n\n"
                    "Esta é uma mensagem automática, mas de muita importância.\n"
                    "Agradecemos pela confiança e estamos à disposição para qualquer dúvida 💜"
                )

            await enviar_mensagem_segura(app.bot, CHAT_ID, msg)

        # ✅ Menu só uma vez, depois de todas as cobranças
        await enviar_mensagem_segura(
            app.bot,
            CHAT_ID,
            "🤖 Comandos disponíveis para interagir comigo:\n"
...
async def enviar_despedida(app):
    mensagem = (
...
def main():
    ...
    # 2. Inicializa o bot normalmente
    app = (
        ApplicationBuilder()
        .token(TOKEN)
        .post_init(avisar_inicio)
        .post_stop(enviar_despedida)
        .connect_timeout(30)   # tolerância maior para conexões lentas
        .build()
    )
    ...
    # 4. Loop de execução com tolerância a falhas de rede
    while True:
        try:
            app.run_polling(close_loop=False)
        except telegram.error.NetworkError as e:
            logging.error(f"Erro de rede: {e}. Tentando novamente em 10s...")
            time.sleep(10)
            continue
